@echo off
setlocal
set COLLECTION=D:\test\angular\Skinet\Scripts\Skinet-Orders.postman_collection\Skinet-Orders.postman_collection.json
set ENV=D:\test\angular\Skinet\Scripts\Skinet-Orders.postman_collection\Skinet-Orders.postman_environment.json
set REPORT_DIR=D:\test\angular\Skinet\Reports
set TS=%date:~6,4%%date:~3,2%%date:~0,2%_%time:~0,2%%time:~3,2%%time:~6,2%
set TS=%TS: =0%
set REPORT_FILE=%REPORT_DIR%\orders-report-%TS%.html
set KEEP_REPORTS=10

echo Avvio test Orders con Newman...
...
for /f "skip=%KEEP_REPORTS% delims=" %%F in ('dir /b /o-d "%REPORT_DIR%\orders-report-*.html"') do del "%REPORT_DIR%\%%F"
endlocal
